...
- (void)setTitleIdArray:(NSArray *)titleIdArray
{
    _titleIdArray = titleIdArray;
}
...
- (void)setTitleArray:(NSArray *)titleArray {
    ...
    [_titleArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (_lastView) {
            menuButton.frame = CGRectMake(_lastView.maxX + 25, 0, 100, self.height);
        }else{
            menuButton.frame = CGRectMake(10, 0, 100, self.height);
        }
        FSLog(@"_titleIdArray为%@",_titleIdArray);
        FSLog(@"_titleIdArray[idx]为%@",_titleIdArray[idx]);
        
        NSInteger i = [_titleIdArray[idx] intValue];
    ...
    self.contentSize = CGSizeMake(CGRectGetMaxX(_lastView.frame)+15, CGRectGetHeight(self.frame));
